...
def _extract_channel_id(channel_url: str) -> str | None:
    ...
    if not channel_url:
        return None
    channel_url = channel_url.strip() #handles leading and trailing spaces in url
...
def _resolve_channel_id(channel_url: str) -> str | None:
    ...
    search_url = "https://www.googleapis.com/youtube/v3/search"#Youtube API Endpoint
    params = {
        #API Parameters
        "part": "snippet",
    ...
    try:
        ...
        if items:
            #capture ChannelId from API Response
...
def _iso_date(date_str: str | None, default_time: str) -> str | None:
    if not date_str:
        return None
    try:
        return f"{date_str}T{default_time}Z" #API COMPATIBLE FORMAT
    except Exception:
        return None
...
def fetch_channel_videos(
    ...
    for idx, v in enumerate(videos):
        ...
        if not v["formatted_transcript"]:
            synthesized = f"Title: {v.get('title','')}. Description: {v.get('description','')}. URL: {v.get('url','')}"#usage of fstrings for all format support w/o .format method or +.
